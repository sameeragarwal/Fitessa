 ...
 * @since		1.5
 */
class JSNISInstallerPlugin extends JObject
{
	/**
	 * Constructor
	 *
	...
	function install()
	{
		...
		$pathInstallFolder = $this->parent->getPath('extension_root').DS.$pname.DS.'install';
		if(JFolder::exists($pathInstallFolder))
		{
			$pathFile = $pathInstallFolder.DS.'install.mysql.sql';
					...
					if (count($queries)) 
					{
						foreach ($queries as $query)
						{
							...
							if ($query != '' && $query{0} != '#') 
							{
								...
								if (!$db->query()) 
								{
									$this->parent->abort(JText::_('Plugin').' '.JText::_('SQL Error').': '.$db->stderr(true));
									return false;
								}
							}
						}
