...
class phpFlickr {
		...
		if ($type == 'db') {
			...
			if ($dir = opendir($this->cache_dir)) {
				while ($file = readdir($dir)) {
					if (substr($file, -6) == '.flickr' && ((filemtime($this->cache_dir . '/' . $file) + $cache_expire) < time()) ) {
	...
	function getCached ($request) {
		...
		if ($this->cache == 'db') {
			...
			//Giang modified
			//$file = dirname(__FILE__).'/'.$this->cache_dir . '/' . $reqhash . '.cache';
			$file = JPATH_CACHE.DS.'jsnim_'.$reqhash . '.flickr';
			if (file_exists($file)) {
	...
	*******************************/
	...
	function call ($method, $arguments) {
		...
		$this->request($method, $arguments, true);
	...
	*/
	...
	function photos_getInfo ($photo_id, $secret = NULL, $cache = false) {
		/* http://www.flickr.com/services/api/flickr.photos.getInfo.html */
		$this->request("flickr.photos.getInfo", array("photo_id"=>$photo_id, "secret"=>$secret), $cache);
		return $this->parsed_response ? $this->parsed_response['photo'] : false;
	}
	...
	function photosets_getList ($user_id = NULL) {
		...
		$this->request("flickr.photosets.getList", array("user_id" => $user_id), true);
