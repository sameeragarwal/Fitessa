...
class SubInstaller extends JObject {
    ...
    function install() {
    	...
    	$checklangB0 = $this->checkFolderLangBOPermission();
    	if($checklangB0 == true){
    	...
    	$checklangF0 = $this->checkFolderLangFOPermission();
    	if($checklangF0 == true){
    	...
    	$checkPlgContent 	= $this->checkWritablePluginContent();
    	...
    	$checkPlgSystem 	= $this->checkWritablePluginSystem();
    	...
    	$checkModule 		= $this->checkWritableModule();    	
      	if($checkModule == true){
	...
	function checkWritablePluginContent()
	{
	...
	function checkWritablePluginSystem()
	{
	...
	function checkFolderLangPermission($base, $nameError)
	{
	...
	function checkFolderLangBOPermission()
	{
		$filepath 	= JPATH_ROOT.DS.'administrator'.DS.'language';
		$this->checkFolderLangPermission($filepath, 'jsn_install_folder_admin');
		$session 	=& JFactory::getSession();
	...
	function checkFolderLangFOPermission()
	{
		$filepath 	= JPATH_ROOT.DS.'language';
		$this->checkFolderLangPermission($filepath, 'jsn_install_folder_client');
		$session 	=& JFactory::getSession();
	...
	function deleteFolderThumbImage()
	{
	    jimport('joomla.filesystem.folder');
